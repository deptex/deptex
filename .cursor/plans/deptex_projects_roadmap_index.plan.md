...
todos:
  ...
  - id: phase-6b-reachability
    ...
    status: pending
  - id: phase-7-aider
    content: "Phase 7: AI-Powered Security Fixing - Aider on Fly.io Machines (poll-based, same pattern as extraction worker) with BYOK keys, fix orchestrator with atom reachability context, 7 strategies across all 11 ecosystems, Jules-like chat-driven planning via Aegis, safety measures (draft PRs, 3-layer timeout, cost cap via Redis INCR, audit trail), global fix status integration with real-time Supabase updates, duplicate fix prevention and Aegis awareness, fix-to-PR lifecycle integration with Phase 8, generalized Fly Machine utility, 22 edge cases with detection/recovery, ~126-test suite"
    status: pending
  - id: phase-7b-aegis-platform
    ...
    status: pending
  - id: phase-8-pr
    content: "Phase 8: PR Management & Webhooks - Manifest registry, smart push extraction (sync_frequency + change detection), daily/weekly extraction scheduler (QStash), watchtower-poller QStash migration, project commits tracking, multi-ecosystem PR analysis, per-project check runs, smart comment system (edit existing), PR tracking table, PR guardrails via Phase 4 policy engine, webhook deliveries audit table, GitLab webhook + MR support, Bitbucket webhook + PR support, repo lifecycle event handling, webhook health display, compliance tab real data (Updates sub-tab), security hardening, worker Fly.io migration strategy, edge cases + error handling + tests"
    status: pending
