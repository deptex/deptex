...
| **r5 (seed=42 temp=0)** | _(in progress)_ | `bench-iterate/seeded-42-r5-temp0/`. ETA ~50 min. Measures whether greedy + seed pins the AI output (→ true variance floor) or whether AI is still nondeterministic on DeepInfra. If r5 ≈ 52, prompt-v2 is the real recall floor. If r5 ≥ 55, temp=0 unlocked stable wins. If r5 ≤ 49, temp=0 hurt (greedy collapses to suboptimal). |
