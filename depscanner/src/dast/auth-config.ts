...
//   recorded → context.authentication{method: browser} + step list (v2.1d)
//   replay   → context.authentication{method: script} + scriptInline body
//              generated from a HAR-derived ReplayCredentialPayload (Phase 36).

// Phase 36 (v1.1) generator for the inline Graal.js script body lives in
// a sibling file so this dispatcher stays focused on the type union +
// per-strategy AF shape. Circular import is type-only on the
// replay-zap-auth.ts side (`import type { ReplayCredentialPayload }`),
// runtime here.
import {
  generateReplayAuthScript,
  ZAP_SCRIPT_ENGINE,
} from './replay-zap-auth';
...
// ---------------------------------------------------------------------------
// Phase 36 (v1.1) — Replay auth (HAR import). Mirror of
// backend/src/types/dast.ts ReplayedRequest + HarTotpStep + ReplayCredentialPayload.
// The shape-coverage test in dast-replay-auth-config.test.ts round-trips every
// optional field to keep this hand mirror honest against the backend source.
// ---------------------------------------------------------------------------
...
/**
 * Phase 36 (v1.1) — builder for the replay auth strategy. Returns the
 * ZAP AF context-authentication shape using `method: 'script'` +
 * `scriptInline` + `scriptEngine` (the M0-resolved Graal.js engine). The
 * yaml-builder consumes the result the same way it does for the form /
 * recorded variants.
 *
 * The `scriptInline` body is generated by `generateReplayAuthScript()` in
 * `replay-zap-auth.ts`. That generator is also tested directly by
 * dast-replay-auth-config.test.ts (shape coverage) + dast-replay-contracts
 * (parseability via `new vm.Script`).
 */
export function buildReplayAuthForZap(
  ...
  const scriptBody = generateReplayAuthScript(payload);
  ...
  return {
    contextAuthentication: {
      method: 'script',
      parameters: {
        scriptEngine: ZAP_SCRIPT_ENGINE,
        scriptInline: scriptBody,
      },
      verification,
    },
    // Script-Based Authentication takes its credentials from the inlined
    // script body itself (cookies / bearers / TOTP secret are all baked
    // in); the empty credentials map exists only to give the requestor
    // job a `user:` to bind to. Without that binding ZAP never invokes
    // the auth method (same empirical finding as the recorded path —
    // see yaml-builder.ts:226-244).
    contextUsers: [
      {
...
export function buildAuthForStrategy(
  ...
  // Phase 36 (v1.1) — replay branch FIRST (cheap discriminator) so the
  // rest of the dispatcher chain stays untouched for the existing v2.1d
  // strategies.
  if (strategy === 'replay') {
    const rep = buildReplayAuthForZap(
      payload as ReplayCredentialPayload,
      loggedInIndicator,
      loggedOutIndicator,
    );
    return {
      contextAuthentication: rep.contextAuthentication,
      contextUsers: rep.contextUsers,
    };
  }
...
export function buildNucleiAuthHeaders(
  ...
  // form / recorded / replay: not expressible as static headers. Nuclei
  // engine cannot drive a multi-step replay or a browser-recorded login.
  // Error message contains the strategy NAME only — no payload bytes.
  throw new UnsupportedAuthStrategyError(strategy);
}
