/**
 * Variant: chain-of-thought-style structured reasoning before the rule.
 *
 * Adds a planning preamble that forces the model to commit to a sink shape,
 * source set, sanitization strategy, and per-fixture metavariable bindings
 * BEFORE authoring the rule_yaml. The Zod schema in generate.ts strips unknown
 * fields, so the planning fields are pure thinking-out-loud forcing functions
 * — they don't change downstream consumers.
 *
 * Same vulnerability section, patch diff, changed-files section, YAML hygiene
 * rules, and few-shot examples as v_base. Only the task framing, output JSON
 * shape, and the new planning preface differ.
 */
...
export const NAME = 'v_cot';
export const VERSION = 'cot-v1';
...
export function buildGenerationPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    ``,
    `# IMPORTANT — Structured analysis BEFORE rule authoring`,
    `Before writing the rule and fixtures, perform a structured analysis. Fill the planning fields FIRST, then the rule_yaml and fixtures. The planning fields force you to commit to a sink shape, source set, and sanitization strategy before authoring; this dramatically reduces "rule doesn't match its own fixture" mistakes.`,
    ``,
    `Planning checklist (these become the leading JSON fields below):`,
    `1. **patch_analysis** — 2–3 sentences: what behavior is the upstream patch fixing? Where in the source-to-sink data flow is the fix?`,
    `2. **sink_shape** — the EXACT call shape Semgrep should match. e.g. \`_.template($STR) — single positional arg, no options\`. Be precise about arity, named args, and method-vs-function form.`,
    `3. **source_shape** — what untrusted input must reach the sink. e.g. \`req.body.* or req.query.*\`.`,
    `4. **sanitization_strategy** — one of: \`static_literal\` (preferred — sink args bound to constants) or \`named_sanitizer\` (only when literal is impossible). State which.`,
    `5. **vulnerable_fixture_plan** — 2 sentences: name the variables that will hold tainted source, name the line where source flows into sink.`,
    `6. **safe_fixture_plan** — 2 sentences: name what each sink metavar binds to in your safe fixture; confirm none binds to \`req.*\`.`,
    ``,
    `Only after the six planning fields are decided, write the rule_yaml and fixtures so they are consistent with the plan.`,
    ``,
    ...
    ``,
    `# Output`,
    `Respond with a SINGLE JSON object. No prose before or after. The shape — fill the planning fields FIRST in this exact order, then the rule artifacts:`,
    '```json',
    `{`,
    `  "patch_analysis": "<2-3 sentences: what behavior is the upstream patch fixing? Where in the source-to-sink data flow is the fix?>",`,
    `  "sink_shape": "<the EXACT call shape Semgrep should match. e.g., '_.template($STR) — single positional arg, no options'>",`,
    `  "source_shape": "<what untrusted input must reach the sink. e.g., 'req.body.* or req.query.*'>",`,
    `  "sanitization_strategy": "<one of: 'static_literal' (preferred — sink args bound to constants) | 'named_sanitizer' (only when literal is impossible). State which.>",`,
    `  "vulnerable_fixture_plan": "<2 sentences: name the variables that will hold tainted source, name the line where source flows into sink>",`,
