/**
 * Prompt-builder variant: v_negfew
 *
 * Adds a NEGATIVE few-shot example (a "looks safe but isn't" trap) immediately
 * before the existing positive few-shot examples section. The trap demonstrates
 * the #1 failure mode in generated rules: pinning one sink metavar to a literal
 * while leaving another sink metavar bound to taint.
 */
...
export const NAME = 'v_negfew';
export const VERSION = 'negfew-v1';
...
export function buildGenerationPrompt(args: BuildPromptArgs): string {
  ...
  const trapSection = renderTrapSection();
...
/**
 * Render the negative few-shot section. This is the v_negfew variant's
 * defining contribution — a worked example of a "looks safe but isn't"
 * fixture that triggers its own rule because the author pinned one sink
 * metavar to a literal but left another bound to taint.
 */
function renderTrapSection(): string {
  return [
    `# Common safe_fixture trap`,
    `Below is a TRAP — a safe_fixture that looks correct but actually triggers the rule. Study the metavar binding carefully; this is the #1 reason generated rules fail validation.`,
    ``,
    `Rule (a real one):`,
    '```yaml',
    `rules:`,
    ...
    '```',
    ``,
    `LOOKS SAFE, ACTUALLY UNSAFE — rule still fires here:`,
    '```js',
    `const filePath = req.body.path;       // tainted`,
    `const pattern = '*.js';                 // literal — pinned, looks safe`,
    `minimatch(filePath, pattern);           // BOOM: rule fires because $PATH binds to req.body.path`,
    '```',
    ``,
    ...
    ``,
    `CORRECT — every metavar in the sink binds to a literal:`,
    '```js',
    `const filePath = 'src/main.js';        // literal`,
    `const pattern = '*.js';                 // literal`,
    `minimatch(filePath, pattern);           // rule does NOT fire — neither $PATH nor $PATTERN binds to taint`,
    '```',
    ``,
    `Apply this to every sink: enumerate every \`$VAR\` and check that the safe_fixture binds it to a literal. Forgetting one metavar = rule fires on its own safe fixture.`,
  ].join('\n');
}

/**
 * Inline a handful of pre-validated rules so the AI mirrors the style we
 * expect. Mirrors the production prompt-builder's renderFewShotSection.
 */
