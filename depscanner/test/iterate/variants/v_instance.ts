/**
 * A/B prompt variant: v_instance
 *
 * Targets the "factory aliasing" failure pattern observed in fixture_pre_match
 * forensics (2 of 13 cases): the model writes patterns pinned to the literal
 * package identifier (`axios.get($URL, ...)`) but real apps alias the package
 * via a factory call (`const instance = axios.create(...); instance.get(url)`),
 * so the rule never matches. Affects axios-style libraries plus pg.Pool,
 * new Client, redis.createClient, mongoose.createConnection, nodemailer
 * .createTransport, etc.
 *
 * Sub-failure: arity mismatch. `axios.get($URL, ...)` (with `...` for the
 * remaining args) sometimes fails to bind a 1-arg call because Semgrep's
 * `...` semantics around the comma vary by version. Best practice is to also
 * include the no-trailing-args form: `axios.get($URL)`.
 *
 * This variant adds two constraints to the existing Constraints block that
 * teach the model to enumerate BOTH the literal-identifier form AND the
 * `$INSTANCE.method(...)` form for any factory-style package, and to pair
 * comma-ellipsis sinks with their no-trailing-args counterpart.
 */
...
export const NAME = 'v_instance';
export const VERSION = 'instance-v1';
...
export function buildGenerationPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    `- IMPORTANT — factory / instance aliasing: When the package exposes a factory or instance constructor (\`axios.create\`, \`new SomeClient(...)\`, \`pg.Pool(...)\`, \`redis.createClient\`, \`mongoose.createConnection\`, \`nodemailer.createTransport\`, etc.), your \`pattern-sinks\` MUST include BOTH the direct form AND the instance form. For example:`,
    '```yaml',
    `pattern-sinks:`,
    ...
    '```',
    `  If the safe_fixture or vulnerable_fixture wouldn't typically alias the package, still include the instance form — real apps almost always alias.`,
    `- IMPORTANT — comma-ellipsis arity: When using Semgrep's \`...\` ellipsis after a comma to match remaining args, ALSO include the no-trailing-args form. The ellipsis-after-comma sometimes fails to bind a 1-arg call. Concrete: instead of writing only \`pattern: 'axios.get($URL, ...)'\`, write both:`,
    '```yaml',
    `pattern-either:`,
    `  - pattern: 'axios.get($URL)'`,
    `  - pattern: 'axios.get($URL, ...)'`,
