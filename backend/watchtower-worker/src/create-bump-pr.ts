...
/**
 * Create a bump PR for a project (used by auto-bump worker; no backend call).
 */
export async function createBumpPrForProject(
  ...
  if (existingBumpPr) {
    return {
      pr_url: (existingBumpPr as { pr_url: string }).pr_url,
      pr_number: (existingBumpPr as { pr_number: number }).pr_number,
    };
  }
  ...
  const { data: repoRow } = await supabase
    ...
    .select('repo_full_name, default_branch, installation_id')
    .eq('project_id', projectId)
    .maybeSingle();
  ...
  const installationId =
    (repoRow as { installation_id?: string }).installation_id ?? orgRow.github_installation_id;
  ...
  if (oldBumpPrs && Array.isArray(oldBumpPrs)) {
    for (const row of oldBumpPrs) {
      try {
        const prState = await getPullRequest(token, repoFullName, (row as { pr_number: number }).pr_number);
        if (prState.state === 'open') {
          await closePullRequest(token, repoFullName, (row as { pr_number: number }).pr_number);
        }
      } catch (e) {
        console.warn('Could not close old bump PR:', (row as { pr_number: number }).pr_number, e);
      }
    }
  }
