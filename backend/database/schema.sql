-- Deptex schema dump
-- Generated by depscanner/scripts/dump-schema.ts
-- Single source of truth for local-mode depscanner (PGLite backend).
...
CREATE TABLE IF NOT EXISTS public.scan_jobs (
  ...
  type text NOT NULL DEFAULT 'extraction'::text,
  target_url text,
  scan_profile text,
...
CREATE TABLE IF NOT EXISTS public.taint_engine_settings (
  ...
  vuln_classes_enabled text[] DEFAULT ARRAY['sql_injection'::text, 'ssrf'::text, 'xss'::text, 'path_traversal'::text, 'command_injection'::text, 'prototype_pollution'::text, 'deserialization'::text, 'redos'::text, 'file_upload'::text, 'open_redirect'::text, 'log_injection'::text, 'code_injection'::text, 'weak_crypto'::text, 'auth_bypass'::text],
...
CREATE OR REPLACE FUNCTION public.fail_exhausted_scan_jobs()
 RETURNS SETOF scan_jobs
 LANGUAGE sql
AS $function$
  UPDATE scan_jobs
...
AS $function$
  SELECT
    p.id AS project_id,
    COALESCE(v.vuln_count, 0), COALESCE(v.critical_count, 0), COALESCE(v.reachable_count, 0), COALESCE(v.worst_depscore, 0),
    COALESCE(v.band_critical,0)+COALESCE(iac.crit,0)+COALESCE(cont.crit,0)+COALESCE(dast.crit,0)+COALESCE(sec.crit,0)+COALESCE(sg.crit,0)+COALESCE(mal.crit,0),
    COALESCE(v.band_high,0)+COALESCE(iac.high,0)+COALESCE(cont.high,0)+COALESCE(dast.high,0)+COALESCE(sec.high,0)+COALESCE(sg.high,0)+COALESCE(mal.high,0),
    COALESCE(v.band_medium,0)+COALESCE(iac.med,0)+COALESCE(cont.med,0)+COALESCE(dast.med,0)+COALESCE(sec.med,0)+COALESCE(sg.med,0)+COALESCE(mal.med,0),
    COALESCE(v.band_low,0)+COALESCE(iac.low,0)+COALESCE(cont.low,0)+COALESCE(dast.low,0)+COALESCE(sec.low,0)+COALESCE(sg.low,0)+COALESCE(mal.low,0),
    COALESCE(ig.ignored_count,0), COALESCE(sgc.semgrep_count,0), COALESCE(secc.secret_count,0), COALESCE(secc.verified_secret_count,0),
    COALESCE(c.has_container,false), COALESCE(d.has_dast,false), sj.last_scan_at
  FROM unnest(p_project_ids) AS p(id)
  LEFT JOIN LATERAL (
    SELECT count(*) AS vuln_count, count(*) FILTER (WHERE r.severity='critical') AS critical_count,
      count(*) FILTER (WHERE r.is_reachable) AS reachable_count, max(r.depscore) FILTER (WHERE r.is_open) AS worst_depscore,
    ...
    FROM (
      SELECT pdv.severity, pdv.is_reachable, pdv.depscore, COALESCE(pdv.contextual_depscore,pdv.depscore,0) AS eff_score,
        NOT (pdv.runtime_confirmed_at IS NULL AND lower(COALESCE(pdv.reachability_level,'')) NOT IN ('confirmed','data_flow')
             AND (lower(COALESCE(pdv.reachability_level,'')) IN ('unreachable','module') OR pdv.is_reachable=false)) AS is_open
      FROM project_dependency_vulnerabilities pdv
      WHERE pdv.project_id=p.id AND pdv.extraction_run_id=ANY(p_active_run_ids) AND pdv.suppressed=false
    ) r
  ) v ON true
  LEFT JOIN LATERAL (
    SELECT count(*) AS ignored_count FROM project_dependency_vulnerabilities pdv
    WHERE pdv.project_id=p.id AND pdv.extraction_run_id=ANY(p_active_run_ids)
      AND (pdv.suppressed=true OR (pdv.runtime_confirmed_at IS NULL AND lower(COALESCE(pdv.reachability_level,'')) NOT IN ('confirmed','data_flow')
        AND (lower(COALESCE(pdv.reachability_level,'')) IN ('unreachable','module') OR pdv.is_reachable=false)))
  ) ig ON true
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (
      SELECT CASE lower(COALESCE(f.severity,'')) WHEN 'critical' THEN 'critical' WHEN 'high' THEN 'high'
               WHEN 'medium' THEN 'medium' WHEN 'moderate' THEN 'medium' WHEN 'low' THEN 'low' ELSE 'low' END AS b
      FROM project_iac_findings f
      WHERE f.project_id=p.id AND f.extraction_run_id=ANY(p_active_run_ids)
        AND COALESCE(f.suppressed,false)=false AND COALESCE(f.risk_accepted,false)=false
        AND (upper(f.rule_id) IN ('CKV_K8S_16','CKV_K8S_17','CKV_K8S_18','CKV_K8S_19','CKV_K8S_20','CKV_K8S_23','KSV-0023','KSV023','AVD-KSV-0023','KSV-0121')
  ...
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (
      SELECT CASE WHEN COALESCE(pcf.depscore,0)>=90 THEN 'critical' WHEN COALESCE(pcf.depscore,0)>=70 THEN 'high'
               WHEN COALESCE(pcf.depscore,0)>=40 THEN 'medium' ELSE 'low' END AS b
      FROM project_container_findings pcf
      WHERE pcf.project_id=p.id AND pcf.extraction_run_id=ANY(p_active_run_ids) AND pcf.is_kev=true AND COALESCE(pcf.suppressed,false)=false
      UNION ALL
      SELECT CASE WHEN m>=90 THEN 'critical' WHEN m>=70 THEN 'high' WHEN m>=40 THEN 'medium' ELSE 'low' END AS b
      FROM (SELECT max(COALESCE(pcf.depscore,0)) AS m FROM project_container_findings pcf
            WHERE pcf.project_id=p.id AND pcf.extraction_run_id=ANY(p_active_run_ids) AND pcf.is_kev=false AND COALESCE(pcf.suppressed,false)=false
            GROUP BY pcf.image_reference) g
    ) q
  ) cont ON true
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE ds>=90) AS crit, count(*) FILTER (WHERE ds>=70 AND ds<90) AS high,
      count(*) FILTER (WHERE ds>=40 AND ds<70) AS med, count(*) FILTER (WHERE ds<40) AS low
    FROM (
      SELECT CASE WHEN g.kev THEN GREATEST(g.ds_raw,96) WHEN g.exploitable THEN GREATEST(g.ds_raw,90) ELSE g.ds_raw END AS ds
      FROM (
        SELECT DISTINCT ON (s.handler_file_path, s.fam) s.ds_raw, s.exploitable, s.kev
        FROM (
          SELECT df.handler_file_path,
            CASE
              ...
              ELSE 'rule:'||lower(COALESCE(df.rule_id, df.vulnerability_type, 'other')) END AS fam,
            LEAST(100, GREATEST(0,
              (CASE lower(COALESCE(df.severity,'')) WHEN 'critical' THEN 90 WHEN 'high' THEN 72 WHEN 'medium' THEN 48 WHEN 'low' THEN 26 WHEN 'info' THEN 10 ELSE 48 END)
              + (CASE lower(COALESCE(df.confidence,'')) WHEN 'confirmed' THEN 10 WHEN 'high' THEN 6 WHEN 'low' THEN -12 ELSE 0 END)
              + (CASE
                   ...
                   ELSE 0 END))) AS ds_raw,
            (df.linked_sca_osv_id IS NOT NULL) AS exploitable, COALESCE(df.kev,false) AS kev,
            ((CASE lower(COALESCE(df.severity,'')) WHEN 'critical' THEN 4 WHEN 'high' THEN 3 WHEN 'medium' THEN 2 WHEN 'low' THEN 1 ELSE 0 END)*10
             + (CASE WHEN lower(COALESCE(df.vulnerability_type,'')||' '||COALESCE(df.message,'')) ~ '(template ?injection|ssti|sql ?injection|sqli|command ?injection)' THEN 3
                     WHEN lower(COALESCE(df.vulnerability_type,'')||' '||COALESCE(df.message,'')) ~ '(persistent|stored)' THEN 2
                     WHEN lower(COALESCE(df.vulnerability_type,'')||' '||COALESCE(df.message,'')) ~ '(cross.?site.?script|\yxss\y)' THEN 1 ELSE 0 END)) AS canon
          FROM project_dast_findings df
          WHERE df.project_id=p.id AND COALESCE(df.status,'open')='open'
            AND df.dast_run_id=(SELECT df2.dast_run_id FROM project_dast_findings df2 WHERE df2.project_id=p.id ORDER BY df2.created_at DESC LIMIT 1)
            AND ((df.payload_redacted IS NOT NULL AND btrim(df.payload_redacted)<>'') OR lower(COALESCE(df.severity,'')) IN ('high','critical'))
        ) s
  ...
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE COALESCE(psf.depscore,0)>=90) AS crit,
    ...
    FROM project_secret_findings psf WHERE psf.project_id=p.id AND psf.extraction_run_id=ANY(p_active_run_ids)
  ) sec ON true
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE COALESCE(sf.depscore,0)>=90) AS crit,
    ...
    FROM project_semgrep_findings sf WHERE sf.project_id=p.id AND sf.extraction_run_id=ANY(p_active_run_ids)
  ) sg ON true
  LEFT JOIN LATERAL (
    SELECT count(*) FILTER (WHERE ds>=90) AS crit, count(*) FILTER (WHERE ds>=70 AND ds<90) AS high,
      count(*) FILTER (WHERE ds>=40 AND ds<70) AS med, count(*) FILTER (WHERE ds<40) AS low
    FROM (SELECT COALESCE(pmf.depscore, CASE WHEN lower(COALESCE(pmf.severity,''))='critical' THEN 95 ELSE 0 END) AS ds
          FROM project_malicious_findings pmf WHERE pmf.project_id=p.id AND pmf.extraction_run_id=ANY(p_active_run_ids)
            AND COALESCE(pmf.suppressed,false)=false AND COALESCE(pmf.risk_accepted,false)=false) q
  ) mal ON true
  LEFT JOIN LATERAL (SELECT count(*) AS semgrep_count FROM project_semgrep_findings sf WHERE sf.project_id=p.id AND sf.extraction_run_id=ANY(p_active_run_ids)) sgc ON true
  LEFT JOIN LATERAL (SELECT count(*) AS secret_count, count(*) FILTER (WHERE psf.is_verified) AS verified_secret_count FROM project_secret_findings psf WHERE psf.project_id=p.id AND psf.extraction_run_id=ANY(p_active_run_ids)) secc ON true
  LEFT JOIN LATERAL (SELECT EXISTS (SELECT 1 FROM project_container_findings pcf WHERE pcf.project_id=p.id AND pcf.extraction_run_id=ANY(p_active_run_ids)) AS has_container) c ON true
  LEFT JOIN LATERAL (SELECT (EXISTS (SELECT 1 FROM project_dast_targets pdt WHERE pdt.project_id=p.id) OR EXISTS (SELECT 1 FROM project_dast_findings pdf WHERE pdf.project_id=p.id)) AS has_dast) d ON true
  LEFT JOIN LATERAL (SELECT max(sj2.completed_at) AS last_scan_at FROM scan_jobs sj2 WHERE sj2.project_id=p.id AND sj2.status='completed') sj ON true;
$function$
;
...
ALTER TABLE public.organization_sla_policies ADD CONSTRAINT uq_sla_policies_org_severity UNIQUE (organization_id, severity);
...
ALTER TABLE public.project_repositories ADD CONSTRAINT project_repositories_organization_id_fkey FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE;
...
CREATE INDEX idx_project_repositories_organization_id ON public.project_repositories USING btree (organization_id);
...
CREATE UNIQUE INDEX idx_project_repositories_org_repo_path ON public.project_repositories USING btree (organization_id, repo_full_name, package_json_path);
