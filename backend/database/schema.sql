...
AS $function$
  SELECT
    p.id AS project_id,
    COALESCE(v.vuln_count, 0), COALESCE(v.critical_count, 0), COALESCE(v.reachable_count, 0),
    COALESCE(v.worst_depscore, 0),
    ...
    COALESCE(ig.ignored_count, 0), COALESCE(sgc.semgrep_count, 0), COALESCE(secc.secret_count, 0),
    COALESCE(secc.verified_secret_count, 0), COALESCE(c.has_container, false), COALESCE(d.has_dast, false),
    sj.last_scan_at
  ...
  LEFT JOIN LATERAL (
    ...
    FROM (SELECT pdv.severity, pdv.is_reachable, pdv.depscore, COALESCE(pdv.contextual_depscore,pdv.depscore,0) AS eff_score,
      ...
      FROM project_dependency_vulnerabilities pdv WHERE pdv.project_id=p.id AND pdv.extraction_run_id=ANY(p_active_run_ids) AND pdv.suppressed=false) r
  ) v ON true
  LEFT JOIN LATERAL (SELECT count(*) AS ignored_count FROM project_dependency_vulnerabilities pdv
    WHERE pdv.project_id=p.id AND pdv.extraction_run_id=ANY(p_active_run_ids)
      AND (pdv.suppressed=true OR (pdv.runtime_confirmed_at IS NULL AND lower(COALESCE(pdv.reachability_level,'')) NOT IN ('confirmed','data_flow')
        AND (lower(COALESCE(pdv.reachability_level,'')) IN ('unreachable','module') OR pdv.is_reachable=false)))) ig ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (SELECT CASE lower(COALESCE(f.severity,'')) WHEN 'critical' THEN 'critical' WHEN 'high' THEN 'high'
        WHEN 'medium' THEN 'medium' WHEN 'moderate' THEN 'medium' WHEN 'low' THEN 'low' ELSE 'low' END AS b
      FROM project_iac_findings f WHERE f.project_id=p.id AND f.extraction_run_id=ANY(p_active_run_ids)
        ...
        AND (upper(f.rule_id) IN ('CKV_K8S_16','CKV_K8S_17','CKV_K8S_18','CKV_K8S_19','CKV_K8S_20','CKV_K8S_23','KSV-0023','KSV023','AVD-KSV-0023','KSV-0121')
          OR (f.framework='dockerfile' AND lower(COALESCE(f.severity,'')) IN ('high','critical')))) q) iac ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (
      ...
      FROM project_container_findings pcf WHERE pcf.project_id=p.id AND pcf.extraction_run_id=ANY(p_active_run_ids)
        AND pcf.is_kev=true AND COALESCE(pcf.suppressed,false)=false
      UNION ALL
      ...
      FROM (SELECT max(COALESCE(pcf.depscore,0)) AS m FROM project_container_findings pcf
        ...
        GROUP BY pcf.image_reference) g) q) cont ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (SELECT CASE WHEN bool_or(lower(COALESCE(df.severity,''))='critical') THEN 'critical'
        WHEN bool_or(lower(COALESCE(df.severity,''))='high') THEN 'high'
        WHEN bool_or(lower(COALESCE(df.severity,''))='medium') THEN 'medium' ELSE 'low' END AS b
      FROM project_dast_findings df WHERE df.project_id=p.id AND COALESCE(df.status,'open')='open'
      ...
      GROUP BY df.handler_file_path, df.handler_line, df.vulnerability_type) q) dast ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE psf.is_verified) AS crit, count(*) FILTER (WHERE NOT psf.is_verified) AS high, 0 AS med, 0 AS low
    FROM project_secret_findings psf WHERE psf.project_id=p.id AND psf.extraction_run_id=ANY(p_active_run_ids)) sec ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (SELECT CASE lower(COALESCE(sf.severity,'')) WHEN 'critical' THEN 'critical' WHEN 'high' THEN 'high' WHEN 'error' THEN 'high'
        WHEN 'medium' THEN 'medium' WHEN 'warning' THEN 'medium' ELSE 'low' END AS b
      FROM project_semgrep_findings sf WHERE sf.project_id=p.id AND sf.extraction_run_id=ANY(p_active_run_ids)) q) sg ON true
  LEFT JOIN LATERAL (SELECT count(*) FILTER (WHERE b='critical') AS crit, count(*) FILTER (WHERE b='high') AS high,
      count(*) FILTER (WHERE b='medium') AS med, count(*) FILTER (WHERE b='low') AS low
    FROM (SELECT CASE lower(COALESCE(pmf.severity,'')) WHEN 'critical' THEN 'critical' WHEN 'high' THEN 'high'
      ...
      FROM project_malicious_findings pmf WHERE pmf.project_id=p.id AND pmf.extraction_run_id=ANY(p_active_run_ids)
        AND COALESCE(pmf.suppressed,false)=false AND COALESCE(pmf.risk_accepted,false)=false) q) mal ON true
  ...
  LEFT JOIN LATERAL (SELECT count(*) AS secret_count, count(*) FILTER (WHERE psf.is_verified) AS verified_secret_count
    FROM project_secret_findings psf WHERE psf.project_id=p.id AND psf.extraction_run_id=ANY(p_active_run_ids)) secc ON true
