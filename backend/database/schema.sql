...
-- ============================================================================
-- phase9z: Aegis Task primitive (hand-reflected from
-- phase9z_aegis_agent_tasks.sql; idempotent so it co-exists with a later real
-- `npm run schema:dump`). Applied to prod via Supabase MCP separately.
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.aegis_agent_tasks (
  ...
  thread_id       uuid REFERENCES public.aegis_chat_threads(id) ON DELETE CASCADE,
  ...
  CONSTRAINT aegis_agent_tasks_kind_chk   CHECK (kind = 'fix')
);
...
-- Legacy aegis-v2 task tables (phase7b) stay dropped on a fresh replay.
