-- phase25a_revert: undo DAST v2.1c — Nuclei engine + runtime-confirmation cross-link.
-- Drops the RPC, restores the narrow scan_jobs CHECK, re-creates the two
-- carry_forward functions WITHOUT the runtime_confirmed_* additions, and drops
-- the additive columns / FK / index.
SET lock_timeout = '5s';

-- 1. Drop the cross-link RPC.
DROP FUNCTION IF EXISTS public.confirm_pdvs_from_dast_run(uuid, text);

-- 2. Restore the narrow scan_jobs_dast_columns_match_type CHECK.
...
-- 3. Drop the FK + covering index, then the three PDV columns and kev.
ALTER TABLE public.project_dependency_vulnerabilities
  DROP CONSTRAINT IF EXISTS project_dependency_vulnerabilities_runtime_confirmed_dast_finding_id_fkey;
DROP INDEX IF EXISTS public.project_dependency_vulnerabilities_runtime_confirmed_fk;
ALTER TABLE public.project_dependency_vulnerabilities
  DROP COLUMN IF EXISTS runtime_confirmed_at,
  DROP COLUMN IF EXISTS runtime_confirmed_dast_finding_id,
  DROP COLUMN IF EXISTS runtime_confirmed_prior_level;
ALTER TABLE public.project_dast_findings DROP COLUMN IF EXISTS kev;

-- 4. Re-create commit_extraction() and finalize_extraction() WITHOUT the
--    carry_forward runtime_confirmed_* additions (pre-phase25a definitions).

-- >>> CARRY_FORWARD_FUNCTIONS_UNPATCHED <<<

-- ===== commit_extraction (pre-phase25a definition) =====
...
-- ===== finalize_extraction (pre-phase25a definition) =====
