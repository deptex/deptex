-- phase52: band every finding type by the SAME depscore the findings table shows
--
-- phase51 made the band pills count all finding types, but it banded the non-SCA
-- types by raw SEVERITY while the unified findings table (VulnerabilityExpandableTable
-- → getRowDepscore + DepscoreValue) bands every row by its reachability-aware DEPSCORE
-- (>=90 crit / >=70 high / >=40 med / <40 low). So the preview pills could disagree with
-- the colours/counts in the table: a secret at depscore 60 showed "medium" in the table
-- but counted as "high" in the pill; a confirmed-exploitable DAST hit scores 90+ (critical)
-- in the table but counted as "high" by severity.
--
-- This re-bands the cleanly-derivable types by the stored/derived depscore so the pills
-- line up with the table:
--   secret    -> stored depscore               (was verified->crit / else high)
--   semgrep   -> stored depscore               (was ERROR->high / WARNING->med)
--   malicious -> stored depscore (crit->95)    (was severity)
--   DAST      -> dastDepscore() ported to SQL  (was severity)
-- SCA (eff_score) and container (depscore) were already depscore-banded — unchanged.
-- The OPEN filters are untouched (they already mirror autoTriageRow): DAST open = has an
-- attack payload OR high/critical severity; secrets/semgrep/malicious all open; container
-- = KEV individually + one base-image row per image.
--
-- NOTE: dastDepscore is mirrored here from backend/src/routes/dast.ts. Both are derived
-- only from stored columns (severity/confidence/vulnerability_type/linked_sca_osv_id/kev),
-- so the mirror is deterministic — but if the TS formula changes, update this too. The
-- permanent fix (store the computed depscore + open-state once so both read it) is the
-- findings-status foundation.
-- IaC stays severity-banded for its critical-rule open set (iacRuleInfo's per-rule score
-- lookup isn't ported); the hardening tail is auto-ignored in both engines, so open IaC is
-- typically empty and this residual rarely shows.
