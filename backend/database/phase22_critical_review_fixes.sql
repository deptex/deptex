-- Phase 22: critical-review fixes for reachability Phase 2 RPCs
--
-- 1. project_entry_points was added in phase20 but never wired into either
--    reaper. reap_old_extractions and reap_orphaned_extractions now mirror
--    the sibling DELETE blocks (project_usage_slices, project_reachable_flows,
--    etc.) so entry-point rows from superseded/failed runs get cleaned up.
--
-- 2. commit_extraction ignored the `namespace` column on project_dependencies
--    (added in phase21) because the jsonb_to_recordset typedef didn't declare
--    it, so the INSERT/UPDATE path silently dropped Maven groupId values.
--    This meant: a re-extraction could collapse two artifacts that share
--    (name, version, is_direct, source) but differ in groupId, and ongoing
--    runs reset the groupId on the UPDATE path because it wasn't propagated.
--    Adding namespace to the typedef, INSERT column list, and DO UPDATE SET
--    restores round-trip fidelity.
--
-- Note: the existing UNIQUE (project_id, name, version, is_direct, source)
-- constraint still doesn't include namespace, so two Maven artifacts with
-- matching (name, version) and different groupIds remain a latent bug.
-- Widening the constraint is deferred — the common case today (each artifact
-- has a distinct name) is unaffected.

-- =============================================================================
-- 1a. reap_old_extractions — add project_entry_points DELETE
-- =============================================================================
...
COMMENT ON FUNCTION reap_old_extractions IS
  'Phase 19.2 (+ Phase 22): hard-delete finding rows belonging to extraction_run_ids that are neither the active nor the previous-active for the given project. Called by commit_extraction at the end of every run and available as a standalone callable. Phase 22 added project_entry_points cleanup.';

-- =============================================================================
-- 1b. reap_orphaned_extractions — add project_entry_points DELETE
-- =============================================================================
...
COMMENT ON FUNCTION reap_orphaned_extractions IS
  'Phase 19.5 (+ Phase 22): hard-deletes rows tagged with extraction_run_ids from failed/cancelled jobs older than p_older_than_hours. Called from the QStash extraction-jobs recovery cron (every 5min). Returns orphan_runs array so the caller can clean up Supabase Storage bucket files. Phase 22 added project_entry_points cleanup.';

-- =============================================================================
-- 2. commit_extraction — add namespace column to the deps upsert path
--    (typedef + INSERT column list + DO UPDATE SET)
-- =============================================================================
...
BEGIN
  ...
  -- ---------------------------------------------------------------------------
  -- Upsert deps — Phase 22: propagate `namespace` end-to-end (was silently
  -- dropped because the jsonb_to_recordset typedef didn't declare it).
  -- ---------------------------------------------------------------------------
  WITH input_deps AS (
...
COMMENT ON FUNCTION commit_extraction IS
  'Phase 19.2 (+ Phase 22): atomic commit of an extraction pipeline run. Upserts deps, soft-deletes missing, inserts findings under fresh extraction_run_id, carries forward user state, detects re-review triggers, writes lifecycle events, computes SLA for new findings, flips active_extraction_run_id pointer, reaps old rows. Returns summary JSONB for notification emission. Phase 22 added namespace propagation on the deps upsert (was silently dropped).';
