/**
 * v_audit — prompt variant that forces the model to perform a structured
 * self-audit of every sink metavariable's binding in its safe_fixture before
 * emitting rule_yaml.
 *
 * Forensic context: 7/11 safe-clean failures in the v_base tournament traced
 * back to a sink metavar (e.g. `$URL`, `$PATTERN`) silently binding to
 * `req.*`-tainted state inside the safe fixture. Adding a required
 * `sink_metavar_audit` field — "for each $VAR, what does it bind to here, and
 * is that a static literal?" — makes the failure mode visible to the model
 * BEFORE it commits to a fixture.
 *
 * The Zod schema in generate.ts uses `.object({ ... })` which strips unknown
 * keys, so the extra `sink_metavar_audit` field travels through the response
 * as a thinking-out-loud channel and is discarded on parse. No production
...
export function buildGenerationPrompt(args: BuildPromptArgs): string {
  ...
  return [
    ...
    ``,
    `# Sink metavar audit (REQUIRED before you commit to rule_yaml + safe_fixture)`,
    `Before writing rule_yaml and safe_fixture, list every \`$VAR\` metavariable that appears in your \`pattern-sinks\` (and \`pattern-sources\` if it matters) and decide what each binds to in your safe_fixture. Every entry MUST have \`is_static_literal: true\` — if you can't make a metavar bind to a literal in your safe_fixture, your rule design is wrong: split into multiple narrower rules or change the sink shape. Concrete: for \`minimatch($PATH, $PATTERN, ...)\`, you must list \`$PATH\` and \`$PATTERN\` and both must bind to literals in the safe fixture.`,
    `Forensic note: most safe-clean failures we've seen come from a sink metavar (e.g. \`$URL\`, \`$PATTERN\`, \`$HEADERS\`) silently binding to \`req.*\` taint in the safe fixture because the author only thought about the *primary* tainted argument. The audit below exists to make every such binding explicit. If a sink has the shape \`fn($A, $B, $C)\`, all three of \`$A\`, \`$B\`, \`$C\` must appear in the audit.`,
    ``,
    `# Output`,
    ...
    `{`,
    `  "sink_metavar_audit": [`,
    `    {"metavar": "$URL", "binds_in_safe_fixture_to": "the literal string 'https://example.com/static-target'", "is_static_literal": true},`,
    `    {"metavar": "$HEADERS", "binds_in_safe_fixture_to": "STATIC_HEADERS object literal", "is_static_literal": true}`,
    `  ],`,
    ...
    `}`,
    '```',
    `Do not wrap the JSON in markdown code fences. Do not include explanations outside the JSON. The rule_yaml field is a string; embed newlines as \\n in JSON. The \`sink_metavar_audit\` array MUST list every metavar that appears in your pattern-sinks; every entry MUST have \`is_static_literal: true\`.`,
  ].join('\n');
}
