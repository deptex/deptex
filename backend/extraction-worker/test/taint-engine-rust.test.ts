/**
 * Validation tests for the Rust substrate of the Deptex cross-file taint
 * engine. Mirrors test/taint-engine-python.test.ts.
 *
 * Run: npx tsx test/taint-engine-rust.test.ts
 */
...
const FIXTURES_ROOT = path.resolve(__dirname, 'taint-engine/fixtures/rust-vulns');
...
const CASES: FixtureCase[] = [
  {
    name: 'Actix SQL injection',
    vulnDir: 'actix-sql-injection-vuln',
    safeDir: 'actix-sql-injection-safe',
    specs: ['actix-web', 'rust-stdlib'],
    expectedVulnClass: 'sql_injection',
  },
  {
    name: 'Actix command injection',
    vulnDir: 'actix-command-injection-vuln',
    safeDir: 'actix-command-injection-safe',
    specs: ['actix-web', 'rust-stdlib'],
...
async function runFixture(c: FixtureCase): Promise<void> {
  ...
  const vulnResult = await propagateRust({ rootDir: vulnRoot, specs });
  ...
  const safeResult = await propagateRust({ rootDir: safeRoot, specs });
...
async function main(): Promise<void> {
  console.log('=== taint-engine rust tests ===');
