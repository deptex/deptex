 ...
 * Can be array of { ranges?: { events: { introduced?, fixed? }[] }[]; versions?: string[] }
 */
type OsvAffected = unknown;

/**
 * Returns true if the given version is affected by the vulnerability
 * according to OSV-style affected_versions (ranges with introduced/fixed events, or versions list).
 */
export function isVersionAffected(version: string, affectedVersions: OsvAffected): boolean {
  // If we have no affected-version data, conservatively assume the vuln applies
...
/**
 * Returns true if the given version is fixed (i.e. not affected) because it is >= one of the fixed versions.
 */
