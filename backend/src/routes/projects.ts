  ...
  try {
    ...
    const formattedProjects = (projects || []).map((project: any) => {
      ...
      return {
        ...
        is_compliant: project.is_compliant !== false, // Default to true if not set
...
router.get('/:id/projects/:projectId/import-status', async (req: AuthRequest, res) => {
  try {
    ...
    // Get unique dependency_ids for this project (status now lives on dependencies table)
    const { data: projectDeps } = await supabase
      .from('project_dependencies')
      .select('dependency_id')
      .eq('project_id', projectId);

    const depIds = [...new Set((projectDeps || [])
      .map((pd: any) => pd.dependency_id)
      .filter(Boolean))];
