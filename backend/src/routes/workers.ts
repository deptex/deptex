...
// Fetches npm info, creates version rows, fetches GHSA vulns, OpenSSF, and calculates reputation score
async function populateSingleDependency(
  ...
  try {
    // Update status to analyzing on the dependencies table
    await supabase
      .from('dependencies')
      .update({ status: 'analyzing', updated_at: new Date().toISOString() })
      .eq('id', dependencyId);

    // 1. Fetch npm registry data (versions, downloads, GitHub URL, timestamps)
    ...
    // 3. Fetch GHSA vulnerabilities for this package
    let ghsaVulnsForPackage: GhsaVuln[] = preFetchedGhsaVulns ?? [];
    if (ghsaVulnsForPackage.length === 0) {
      const m = await fetchGhsaVulnerabilitiesBatch([name]);
...
router.post('/queue-populate', verifyWorkerSecret, async (req, res) => {
  const { projectId, organizationId, dependencies } = req.body || {};
  ...
  const deps = (dependencies as Array<{ dependencyId: string; name: string }>)
    .filter((d) => d.dependencyId && d.name)
    .map((d) => ({ dependencyId: d.dependencyId, name: d.name }));
...
router.post('/populate-dependencies', verifyQStash, async (req, res) => {
  ...
  if (populateDependenciesInFlight) {
    // Another batch is still running; QStash will retry this message later
  ...
  try {
    console.log(`Populating batch of ${dependencies.length} dependencies`);

    // One GHSA GraphQL batch query for up to 100 packages (same source as npm audit)
    ...
    for (let i = 0; i < uniqueNames.length; i += 100) {
      ...
      const m = await fetchGhsaVulnerabilitiesBatch(chunk);
      m.forEach((vulns, name) => ghsaBatch.set(name, vulns));
    }
    if (uniqueNames.length > 0) {
      console.log(`[GHSA] Fetched vulns for ${uniqueNames.length} packages in ${numChunks} request(s) (batch)`);
    }

    const results: Array<{ name: string; success: boolean; score?: number; error?: string }> = [];

    // Process dependencies sequentially (npm + OpenSSF per package)
    for (const dep of dependencies) {
      ...
      const result = await populateSingleDependency(dep.dependencyId, dep.name, preFetched);
      results.push({
        name: dep.name,
        ...result,
      });

      // Delay between packages to avoid npm 429s (registry + downloads per package)
