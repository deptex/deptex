...
4. **Source-of-truth files** (in the worktree — compaction-safe, these ARE the loop's memory):
   - `.claude/plans/reachability-loop-LEDGER.md` — the module-finding ledger + iteration journal. One row per module finding: `repo · dep · CVE · verdict · root_cause · ground_truth · fix (commit SHA or deferral reason) · before→after level`. Plus a per-iteration `## Iteration N` section.
